...
describe('App', () => {
  ...
  describe('hamburgerChange', () => {
    it('should set state if hamburger is closed and state is ready', () => {
      ...
      expect(wrapper.state().hamburger).toEqual({"bottomBladeToggled": "bottom-blade-toggled",
  ...
  describe('pullVehicleData', () => {
    it('should check localStorage when pullVehicleData is called and set state', () => {
      localStorage.setItem('vehicles', JSON.stringify([{test: 'test'}]));
      
      wrapper.instance().pullVehicleData();
  ...
  describe('fetchVehicleData', () => {
    it('should set state after fetchVehicleData is called', async () => {
      const mockFetch = jest.fn(() => {
        return Promise.resolve([{vehicle: 'car'}, {vehicle: 'boat'}]);
      });
